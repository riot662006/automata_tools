...
class DFAV2:
    def __init__(
        ...
        self.edges_cache: Optional[Dict[str, Dict[str, Tuple[str, ...]]]] = None
    ...
    def _add_transitions(self, transitions: Mapping[Tuple[str, str], str]) -> None:
        ...
        for (src, sym), dst in transitions.items():
            self.tx.set(self._sid_of(src), self._aid_of(sym), (self._sid_of(dst),))
        self.dirty_edges = True
    ...
    def edit(self):
        ...
        try:
            ...
            if not self.is_valid_dfa():
                raise ValueError("Invalid DFA after edit() — each (state, symbol) must have exactly one destination.")
